...
What is sold: coin codes for an AI co-op buddy that plays classic NES games together with the buyer in the browser. Price: ${cfg.service.price} ${cfg.service.currency} per coin (1 coin = ${getSessionMinutes()} minutes of play on a clock that starts when the coin is inserted and NEVER pauses, not even if the browser is closed or crashes; the buyer can leave and come back with the same code for free as long as that clock is still running, and once it runs out the next Insert Coin spends the next coin; buying for N ${cfg.service.currency} gives N coins on one code). After the order is funded the code and the play link are delivered automatically within minutes — no human in the loop.
