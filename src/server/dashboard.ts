...
async function load(){
  ...
  $('perCoinNote').textContent='Estimate = Jev tokens × $'+pc.priceInPerM+'/M in, $'+pc.priceOutPerM+'/M out (JEV_PRICE_*_PER_M), booked when a browser session ends. "Bill ÷ coins" is the relay key\'s all-time usage divided by all coins ever inserted, so it also carries anything else the key paid for.';
